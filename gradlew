...
if $cygwin ; then
    ...
    if [ -n "$ROOTDIRSRAW" ]; then
        ...
        PAT="^(($ROOTDIRS))"
        if ! echo "$APP_HOME" | grep -E "$PAT" >/dev/null 2>&1 ; then
            APP_HOME=`cygpath --path --mixed "$APP_HOME"`
        fi
        PAT="^(($ROOTDIRS))"
        if ! echo "$CLASSPATH" | grep -E "$PAT" >/dev/null 2>&1 ; then
            CLASSPATH=`cygpath --path --mixed "$CLASSPATH"`
        fi
    fi
fi
...
eval set -- "$DEFAULT_JVM_OPTS" "$JAVA_OPTS" "$GRADLE_OPTS" "\"$APP_HOME/gradle/wrapper/gradle-wrapper.jar\"" "$@"

# by default we should be in the correct project dir
# but when run from Finder on macOS, the cwd is wrong
if [ "$(uname)" = "Darwin" ] && [ "$4" = "-classpath" ] ; then
    # remove the -classpath argument and the classpath
    shift 2
fi
